...
class ImageResourceHandler(ImageApiHandler):
    async def check_resource(self, id):
        id = id[: self.context.config.MAX_ID_LENGTH]
        # Check if image exists
        exists = await self.context.modules.storage.exists(id)

        if exists:
            body = await self.context.modules.storage.get(id)
            self.set_status(200)
    ...
    async def put(self, id):
        ...
        if self.validate(self.request.body):
            await self.write_file(id, self.request.body)
            self.set_status(204)
    ...
    async def delete(self, id):
        ...
        exists = await self.context.modules.storage.exists(id)
        if exists:
            await self.context.modules.storage.remove(id)
            self.set_status(204)
        else:
            self._error(404, "Image not found at the given URL")

    async def get(self, id):
        await self.check_resource(id)

    async def head(self, id):
        await self.check_resource(id)
