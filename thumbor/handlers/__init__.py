...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def on_finish(self, *args, **kwargs):
        ...
        self.context.metrics.timing(
            "response.time.{0}".format(status), total_time
        )
        ...
        if self._response_ext is not None:
            ...
            self.context.metrics.timing(
                "response.time{0}".format(ext), total_time
            )
        ...
        if self.context.modules.result_storage and should_store:
            ...
            try:
                ...
                self._error(
                    ...
                    "from the result storage: {}".format(e),
                )
                return
    ...
    async def get_image(self):
        ...
        try:
            ...
            msg = (
                "[BaseHandler] get_image failed for url `{url}`."
                " error: `{error}`"
            ).format(url=self.context.request.image_url, error=e)
    ...
    def can_auto_convert_png_to_jpg(self):
        ...
        enabled = (
            config.AUTO_PNG_TO_JPG and request_override is None
        ) or request_override
    ...
    def define_image_type(self, context, result):
        if result is not None:
            ...
            if image_extension is not None:
                ...
                logger.debug(
                    ...
                    "from the image extension: %s." % image_extension
                )
        ...
        if context.request.meta:
            ...
            logger.debug(
                "Metadata requested. Serving content type of %s."
                % content_type
            )
    ...
    async def finish_request(self, result_from_storage=None):
        ...
        try:
            ...
            self._error(
                500, "Error while trying to fetch the image: {}".format(e)
            )
            return
    ...
    def reload_to_fit_in_kb(
        self, engine, initial_results, extension, initial_quality, max_bytes
    ):
        if (
        ...
        while len(results) > max_bytes:
            ...
            if quality < 10:
                logger.debug(
                    ...
                    "required size of %d bytes." % max_bytes
                )
                return initial_results
    ...
    async def _fetch(self, url):
        ...
        try:
            ...
            if (
                not fetch_result.successful
                and fetch_result.exception is not None
            ):
...
class ImageApiHandler(ContextHandler):
    ...
    async def write_file(self, id, body):
