...
Example:
    ...
    async def on_request_received_handler(sender, request):
        # do something
        return 'done'
...
class Events(object):
    ...
    def scheduler(future):
        ...
        if future.__class__.__name__ == "coroutine":
            future = asyncio.create_task(future)

        fut = tornado.platform.asyncio.to_tornado_future(future)
    ...
    @classmethod
    async def trigger(cls, event, sender, **kw):
