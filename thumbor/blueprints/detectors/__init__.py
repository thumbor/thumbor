...
@gen.coroutine
def get_features(details, key, cascade_file_path):
    cascade = CascadeRegistry.instance().get_cascade(key, cascade_file_path)
    img = yield Engine.convert_to_grayscale(
        'detectors', details, update_image=False, with_alpha=False)
    img = np.array(img)
    size = yield Engine.get_image_size('detectors', details)

    faces = cascade.detectMultiScale(
        img, 1.2, 4, minSize=get_min_size_for(size))
    faces_scaled = []

    for (pos_x, pos_y, width, height) in faces:
        faces_scaled.append(((pos_x.item(), pos_y.item(), width.item(),
                              height.item()), 0))
