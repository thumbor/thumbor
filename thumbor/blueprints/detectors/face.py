...
async def on_smart_detect(sender, request, details):
    cascade_path = details.config.FACE_DETECTOR_CASCADE_FILE

    if cascade_path is None:
        cascade_path = join(dirname(__file__), "haarcascade_frontalface_alt.xml")
    features = await get_features(details, "local-face-detect", cascade_path)
