...
def plug_into_lifecycle():
    ...
    Events.subscribe(
        Events.Imaging.before_loading_source_image, on_before_loading_source_image
    )
    Events.subscribe(
        Events.Imaging.after_loading_source_image, on_after_loading_source_image
    )
...
def is_expired(details, path):
    ...
    expire_in_seconds = details.config.get("RESULT_STORAGE_EXPIRATION_SECONDS", None)
...
async def on_after_parsing_argument(
    sender, request, details
...
async def on_before_loading_source_image(
    sender, request, details
):  # pylint: disable=unused-argument
...
async def on_after_loading_source_image(
    sender, request, details
):  # pylint: disable=unused-argument
...
async def on_after_finish_request(
    sender, request, details
):  # pylint: disable=unused-argument
