...
fn reflected_index(index: isize, size: usize) -> usize {
    if size <= 1 {
        return 0;
    }

    let period = (2 * size - 2) as isize;
    let mut reflected = index % period;
    if reflected < 0 {
        reflected += period;
    }
    if reflected >= size as isize {
...
fn hat_transform(temp: &mut [f32], base: &[f32], st: usize, size: usize, sc: usize) {
    for (i, value) in temp.iter_mut().enumerate().take(size) {
        let center = st * i;
        let left = st * reflected_index(i as isize - sc as isize, size);
        let right = st * reflected_index(i as isize + sc as isize, size);
        *value = 2.0 * base[center] + base[left] + base[right];
    }
}
...
fn apply<'py>(
    ...
    if width <= 0 || height <= 0 {
        return Err(value_error("Sharpen dimensions must be positive."));
    }

    let num_bytes = bytes_per_pixel(&image_mode_str);
    if num_bytes < CHANNELS {
        return Err(value_error("Sharpen requires an RGB or RGBA image."));
    }

    let expected_buffer_size = (width as usize)
        .checked_mul(height as usize)
        .and_then(|pixels| pixels.checked_mul(num_bytes))
        .ok_or_else(|| value_error("Sharpen dimensions are too large."))?;
    if buffer_py.len() < expected_buffer_size {
    ...
    PyBytes::new_with(py, buffer_py.len(), |buffer| {
        buffer.copy_from_slice(buffer_py);
...
#[cfg(test)]
mod tests {
    use super::{hat_transform, reflected_index};

    #[test]
    fn reflected_indices_stay_in_bounds_for_small_sizes() {
        for size in [1, 2, 15, 16, 31, 32] {
            for index in -64..64 {
                assert!(reflected_index(index, size) < size);
            }
        }
    }

    #[test]
    fn hat_transform_accepts_small_dimensions() {
        for size in [1, 2, 15, 16, 31, 32] {
            let base = vec![1.0f32; size];
            let mut temp = vec![0.0f32; size];

            for sc in [1, 2, 4, 8, 16] {
                hat_transform(&mut temp, &base, 1, size, sc);
            }

            assert!(temp.iter().all(|value| value.is_finite()));
        }
    }
}
