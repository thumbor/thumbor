...
const MAX_CONVOLUTION_TAPS: usize = 301;
const MAX_CONVOLUTION_WORK: usize = 1_000_000_000;

fn value_error(message: &str) -> PyErr {
    PyErr::new::<pyo3::exceptions::PyValueError, _>(message.to_string())
}

fn validate_work(width: usize, height: usize, kernel_size: usize) -> PyResult<()> {
    if width == 0 || height == 0 || kernel_size == 0 {
        return Err(value_error("Invalid kernel size or dimensions."));
    }
    if kernel_size > MAX_CONVOLUTION_TAPS {
        return Err(value_error("Convolution kernel is too large."));
    }

    let pixels = width
        .checked_mul(height)
        .ok_or_else(|| value_error("Convolution dimensions are too large."))?;
    let work = pixels
        .checked_mul(kernel_size)
        .ok_or_else(|| value_error("Convolution kernel is too expensive."))?;
    if work > MAX_CONVOLUTION_WORK {
        return Err(value_error("Convolution kernel is too expensive."));
    }
...
fn apply(
    ...
    if !kernel_size.is_multiple_of(columns_count)
        || kernel_size.is_multiple_of(2)
        || rows_count.is_multiple_of(2)
    {
        return Err(value_error("Invalid kernel size or dimensions."));
    }
    ...
    validate_work(width, height, kernel_size)?;
    let expected_buffer_size = width
        .checked_mul(height)
    ...
    PyBytes::new_with(py, buffer.len(), |out_buffer| {
        for y in 0..height {
