...
#[pyfunction]
fn apply(py: Python<'_>, image_mode_str: &str, buffer: &[u8]) -> PyResult<Py<PyBytes>> {
    ...
    PyBytes::new_with(py, buffer.len(), |out| {
        ...
        let red_hist = compute_histogram(r_idx, out, pixel_size);
        let green_hist = compute_histogram(g_idx, out, pixel_size);
        let blue_hist = compute_histogram(b_idx, out, pixel_size);

        let (red_cdf, r_min) = compute_cdf(&red_hist);
        let (green_cdf, g_min) = compute_cdf(&green_hist);
        let (blue_cdf, b_min) = compute_cdf(&blue_hist);

        out.chunks_exact_mut(pixel_size).for_each(|chunk| {
