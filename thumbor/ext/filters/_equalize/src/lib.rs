...
fn apply(py: Python<'_>, image_mode_str: &str, buffer: &[u8]) -> Py<PyBytes> {
    ...
    while i + num_bytes <= buffer.len() {
        ...
        buffer[i + r_idx] = equalize_value(red_cum[r], r_min_cdf, area);
        buffer[i + g_idx] = equalize_value(green_cum[g], g_min_cdf, area);
        buffer[i + b_idx] = equalize_value(blue_cum[b], b_min_cdf, area);
