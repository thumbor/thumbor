...
fn get_curve(points: &Bound<'_, PyTuple>) -> PyResult<Vec<u8>> {
    ...
    for p in points.iter() {
        let pair = p.get_item(0)?;
        let x: u8 = pair.extract()?;
        let y: u8 = p.get_item(1)?.extract()?;
        items.push(x);
        items.push(y);
    }
    Ok(items)
...
fn cubic_spline_interpolation(points: &[u8], count_p: usize, size: usize) -> Vec<u8> {
    ...
    for i in 0..(count_p - 1) {
        ...
        let sd_i = calculate_second_derivative(points, count_p).unwrap_or(vec![0.0; count_p])[i];
        let sd_i_next =
            calculate_second_derivative(points, count_p).unwrap_or(vec![0.0; count_p])[i + 1];
        ...
        for x in current_x..next_x {
            ...
            let y = a * current_y as f64
                + b * next_y as f64
                + (h * h / 6.0) * ((a * a * a - a) * sd_i + (b * b * b - b) * sd_i_next);

            items[x as usize] = y.round().clamp(0.0, 255.0) as u8;
        }
    }
