...
from tests.fixtures.images import (alabama1, animated_image, default_image,
                                   invalid_quantization, not_so_animated_image,
                                   space_image)
    ...
    async def test_image_already_generated_by_thumbor_2_times(self):
        with open(
            normalize_unicode_path(
                u"./tests/fixtures/images/alabama1_ap620é.jpg"
            ),
            "rb",
        ) as f:
            await self.context.modules.storage.put(
                quote("http://test.com/smart/alabama1_ap620é"), f.read()
            )
        crypto = CryptoURL("ACME-SEC")
        image_url = self.get_url(
            crypto.generate(
...
class ImageOperationsWithAutoWebPWithResultStorageTestCase(
    ...
    async def test_can_auto_convert_jpeg_from_result_storage(
        ...
        with open("./tests/fixtures/images/image.webp", "rb") as f:
            await self.context.modules.result_storage.put(f.read())
...
class ImageOperationsResultStorageOnlyTestCase(BaseImagingTestCase):
    ...
    @gen_test
    async def test_loads_image_from_result_storage_fails_on_exception(
        self, get_mock_1
    ):
        response = await self.async_fetch(
            "/gTr2Xr9lbzIa2CT_dL_O0GByeR0=/animated.gif"
...
class EngineLoadException(BaseImagingTestCase):
    ...
    @gen_test
    async def test_should_error_on_engine_load_exception(self, load_mock):
    ...
    async def test_should_optimize_jpeg(self):
        ...
        if not exiftool:
            raise AssertionError(
                "exiftool was not found. Please install it "
                "to run thumbor's tests."
            )
        ...
        try:
            ...
            expect(output).to_equal(
                "Encoding Process                : Progressive DCT, Huffman "
                "coding\n"
            )
        finally:
...
class LoaderErrorTestCase(BaseImagingTestCase):
    ...
    async def test_should_propagate_custom_loader_error(self):
        ...
        async def load_override(context, path):
            result = LoaderResult()
