...
from thumbor.doctor import check_modules, format_error, run_doctor
...
def test_format_error_indents_multiline_text():
    result = format_error(
        "cairosvg",
        "Cannot load CairoSVG\nMissing libcairo-2.dll",
        "Install the native Cairo library\nThen run thumbor-doctor again",
    )

    assert "        Cannot load CairoSVG\n        Missing libcairo-2.dll" in (
        result
    )
    assert (
        "        Install the native Cairo library\n"
