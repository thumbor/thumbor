...
For information on all built-in detectors check the {doc}`available_detectors`
page.

## Configuration

In order to tell thumbor what detectors it should run in the original image, you
must add them to your `thumbor.conf` file in the following key:
...
detection and the feature detection. These are mutually exclusive, meaning that
if a face is detected, the feature detector won't be run.

## Using it

After restarting thumbor, it should be as easy as adding a `/smart` option to
your URLs, like:
...
sure of why an image was cropped the way it was.
````

## Lazy Detection

Facial detection can be pretty expensive for thumbor, so it is not advisable to
do it synchronously. Please refer to the {doc}`lazy_detection` page for
