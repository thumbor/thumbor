...
Filters are an easy way to transform images using a pipeline. Creating a new
filter is very simple, as we'll see.

The first step is creating a filter class that inherits from
`thumbor.filters.BaseFilter` and naming it `Filter`:
...
the `quality(99)` filter, a filter that takes a `number` parameter and sets the
image quality to that parameter.
